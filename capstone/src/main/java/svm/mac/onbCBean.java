...
public class onbCBean implements Serializable, ISearch, IToolBar {
    ...
    // Injected framework dependencies
    @Inject
    ...
    public void init() {
        ...
        model = new ONBModel();
        model.init(); // REQUIRED (no @PostConstruct)
    ...
    public boolean btnSaveActionPerformed(ActionEvent ae) {
        ...
        // 🔑 UPDATE MODE → always allow save
        if (model.getDataModel().isEditMode()) {
            System.out.println("✏️ Edit mode save");
            ctrl.btnSave();
            return true;
        }

        // 🔑 INSERT MODE → allow only if new rows exist
        if (!model.getDataModel().hasNewRows()) {
