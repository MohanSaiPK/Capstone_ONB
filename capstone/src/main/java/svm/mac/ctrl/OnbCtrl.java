...
public class OnbCtrl implements Serializable {
    ...
    public void btnSave() {
        ...
        if (!dm.isEditMode() && hasNewRows()) {
            ...
            if (!saved) {
                return; //  DO NOT show "Save successful"
            }
            ...
            FacesContext.getCurrentInstance().addMessage(
                    ...
                    new FacesMessage(
                            FacesMessage.SEVERITY_INFO,
                            "ONB Onboard Report",
                            "Inspection details have been saved successfully."));
            RequestContext.getCurrentInstance().update("growl");
        ...
        if (!dm.isEditMode() && !hasNewRows()) {
            FacesContext.getCurrentInstance().addMessage(
                    ...
                    new FacesMessage(
                            FacesMessage.SEVERITY_INFO,
                            "ONB Onboard Report",
                            "No new inspection records to save"));
            RequestContext.getCurrentInstance().update("growl");
            System.out.println("ℹ No new rows to save in insert mode");
            return;
        }

        /*
         * =====================================================
         * UI-only row edited → treat as INSERT, not UPDATE
         * =====================================================
         */
        ONBInspectionRowGet selected = dm.getSelectRow();
        ...
        if (dm.isEditMode() && selected != null && selected.isNewlyadded()) {
            ...
            System.out.println("🆕 Editing UI-only row → INSERT instead of UPDATE");

            saveInspection(); // reuse existing insert logic
        ...
        /*
         * =====================================================
         * EDIT MODE OR FORM-SUBMIT MODE → VALIDATION REQUIRED
         * =====================================================
         */
        if (!validateForm()) {
            return;
        ...
        FacesContext.getCurrentInstance().addMessage(
                ...
                new FacesMessage(
                        FacesMessage.SEVERITY_INFO,
                        "ONB Onboard Report",
                        "Inspection details have been updated successfully."));
        RequestContext.getCurrentInstance().update("growl");
    ...
    private boolean invalid(String msg, Runnable flag) {
        flag.run();
        FacesContext.getCurrentInstance()
                .addMessage(null, new FacesMessage(FacesMessage.SEVERITY_WARN, "ONB Onboard Report", msg));
        RequestContext.getCurrentInstance().update("growl");
        return false;
    }
    ...
    private boolean validateForm() {
        ...
        if (isEmpty(dm.getInspectionCode()))
            return invalid("Type of Inspection is required", () -> dm.setInspectionCodeInvalid(true));

        if (dm.getDateOfInspection() == null)
            return invalid("Inspection Date is required", () -> dm.setDateOfInspectionInvalid(true));

        if (isEmpty(dm.getPlaceOfInspection()))
            return invalid("Place of Inspection is required", () -> dm.setPlaceOfInspectionInvalid(true));

        if (isEmpty(dm.getFromPortCode()))
            return invalid("From Port is required", () -> dm.setFromPortCodeInvalid(true));

        if (isEmpty(dm.getToPortCode()))
            return invalid("To Port is required", () -> dm.setToPortCodeInvalid(true));

        if (dm.getFromPortCode().equals(dm.getToPortCode()))
            return invalid("From Port and To Port cannot be the same", () -> {
                dm.setFromPortCodeInvalid(true);
                dm.setToPortCodeInvalid(true);
            });
            String inspector = dm.getInspectorName();
        if (inspector != null) {
            dm.setInspectorName(inspector.trim());
        }

            if (isEmpty(inspector))
            return invalid("Inspector name is required", () -> dm.setInspectorNameInvalid(true));
        //  LETTERS ONLY VALIDATION (ADD THIS)
        if (!inspector.matches("^[A-Za-z]+$"))
            return invalid("Inspector name must contain only letters",
            ...
            if (isSameInspection(existing, newRow)) {
                ...
                FacesContext.getCurrentInstance().addMessage(
                        ...
                        new FacesMessage(
                                ...
                                "ONB Onboard Report",
                                "Same inspection details already exist"));
            ...
            if (isSameInspection(selected, temp)) {
                FacesContext.getCurrentInstance().addMessage(
                        ...
                        new FacesMessage(
                                FacesMessage.SEVERITY_WARN,
                                "ONB Onboard Report",
                                "No changes detected. Modify data or click Cancel."));
                RequestContext.getCurrentInstance().update("growl");
                return false;
            }
            ...
            if (isSameInspection(row, candidate)) {
                FacesContext.getCurrentInstance().addMessage(
                        ...
                        new FacesMessage(
                                FacesMessage.SEVERITY_WARN,
                                "ONB Onboard Report",
                                "An inspection with the same details already exists"));
                RequestContext.getCurrentInstance().update("growl");
                return true;
            }
        }
