...
public class OnbCtrl implements Serializable {
    ...
    private boolean isFormFilled(ONBDataModel dm) {
        return !isEmpty(dm.getInspectionCode())
                || dm.getDateOfInspection() != null
                || !isEmpty(dm.getPlaceOfInspection())
                || !isEmpty(dm.getFromPortCode())
                || !isEmpty(dm.getToPortCode())
                || !isEmpty(dm.getInspectorName());
    }
